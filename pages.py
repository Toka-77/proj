...
class DashboardPage:
    def __init__(self):
        self.page, lay = page_container(tr("Dashboard"), tr("Real-time Overview"))
        row = QHBoxLayout(); row.setSpacing(14)
        self.s_rooms = StatCard("🏠", tr("Total Rooms"), "0", "#7c5cbf")
        self.s_occ   = StatCard("🔴", tr("Occupied"),    "0", "#f06292")
        self.s_rev   = StatCard("💰", tr("Revenue"),  tr("0 EGP"), "#3ecf8e")
        self.s_prof  = StatCard("📈", tr("Net Profit"),tr("0 EGP"), "#5c9cf5")
        ...
        lbl2 = QLabel(tr("⏱  Active Sessions")); lbl2.setObjectName("sec_title")
        lay.addWidget(lbl2)
        self.table = make_table(["#", tr("Room"), tr("Type"), tr("Customer"), tr("People"), tr("Started"), tr("Elapsed")])
        lay.addWidget(self.table)

    def refresh(self):
        rooms = RoomManager.get_all_rooms()
        occ = sum(1 for r in rooms if r[3] == tr("Occupied"))
...
class RoomsPage:
    def __init__(self):
        ...
        self.rooms_tbl = make_table(["#",tr("Name"),tr("Type"),tr("Status"),tr("Price/hr"),tr("Capacity")])
        ...
        rl.addWidget(QLabel(tr("▶  Start Session"), objectName="sec_title"))
        f1 = QFormLayout(); f1.setSpacing(10)
        self.start_room = QComboBox()
        self.start_cust = QLineEdit(); self.start_cust.setPlaceholderText(tr("Customer name..."))
        ...
        rl.addLayout(f1)
        b1 = QPushButton(tr("▶  Start Session")); b1.setObjectName("primary")
        b1.setCursor(Qt.PointingHandCursor); b1.clicked.connect(self.handle_start)
        rl.addWidget(b1)

        rl.addWidget(make_divider())

        rl.addWidget(QLabel(tr("⏹  End Session"), objectName="sec_title"))
        ...
        rl.addLayout(f2)
        b2 = QPushButton(tr("⏹  End & Generate Bill")); b2.setObjectName("danger")
        ...
        lay.addWidget(QLabel(tr("📋  Session History"), objectName="sec_title"))
        self.hist_tbl = make_table(["#",tr("Room"),tr("Type"),tr("Customer"),tr("People"),tr("Start"),tr("End"),tr("Room $"),tr("Snacks"),tr("Disc"),tr("Total")])
        self.hist_tbl.setMaximumHeight(200)
        lay.addWidget(self.hist_tbl)
        
        pdf_btn = QPushButton(tr("📄 Export Session Invoice (PDF)"))
    ...
    def refresh(self):
        ...
        for i, r in enumerate(rooms):
            ...
            if si:
                si.setForeground(QColor("#3ecf8e") if r[3]==tr("Available") else QColor("#f06292"))
            if r[3] == tr("Available"):
    ...
    def handle_start(self):
        ...
        if not rid:
            return QMessageBox.warning(self.page, tr("Error"), tr("No available room."))
            
        cname = self.start_cust.text().strip()
        if not cname:
            return QMessageBox.warning(self.page, tr("Required"), tr("Enter a customer name."))
    ...
    def handle_end(self):
        ...
        if not sid:
            return QMessageBox.warning(self.page, tr("Error"), tr("No active session."))
    ...
    def handle_edit_room(self):
        ...
        if row < 0 or row >= len(self._room_ids):
            return QMessageBox.warning(self.page, tr("Error"), tr("Select a room from the table first."))
        rid = self._room_ids[row]
        rname = self.rooms_tbl.item(row, 1).text()
        curr_price = float(self.rooms_tbl.item(row, 4).text())
        
        new_price, ok = QInputDialog.getDouble(self.page, tr("Edit Room Price"), f"Enter new hourly price for {rname}:", curr_price, 0, 10000, 2)
        if ok:
    ...
    def handle_pdf(self):
        ...
        if row < 0 or row >= len(getattr(self, '_hist_ids', [])):
            return QMessageBox.warning(self.page, tr("Error"), tr("Select a session from the history table."))
    ...
    def handle_add_room(self):
        ...
        name_e = QLineEdit(); name_e.setPlaceholderText(tr("e.g. Study Room C"))
        type_cb = QComboBox(); type_cb.addItems([tr("Study"), tr("Gaming"), tr("Cinema")])
        ...
        desc_e  = QLineEdit(); desc_e.setPlaceholderText(tr("Optional description"))
        fl.addRow(tr("Name:"), name_e)
        fl.addRow(tr("Type:"), type_cb)
        fl.addRow(tr("Price/hr:"), price_e)
        fl.addRow(tr("Capacity:"), cap_e)
        fl.addRow(tr("Description:"), desc_e)
        ...
        if dlg.exec_() == QDialog.Accepted:
            ...
            if not name:
                return QMessageBox.warning(self.page, tr("Error"), tr("Room name is required."))
            ok, msg = RoomManager.add_room(name, type_cb.currentText(), price_e.value(), cap_e.value(), desc_e.text().strip())
            QMessageBox.information(self.page, "✅", msg) if ok else QMessageBox.warning(self.page, tr("Error"), msg)
            self.refresh()

    # ═══ INVENTORY ═══
class InventoryPage:
    def __init__(self):
        self.page, lay = page_container(tr("Snacks & Inventory"))
        grid = QHBoxLayout(); grid.setSpacing(20)

        left = QWidget(); left.setObjectName("card")
        ll = QVBoxLayout(left); ll.setContentsMargins(16,16,16,16)
        hdr_layout = QHBoxLayout()
        hdr_layout.addWidget(QLabel(tr("📦  Products Catalogue"), objectName="sec_title"))
        hdr_layout.addStretch()
        self.inv_val_lbl = QLabel(tr("Total Value: 0.00 EGP"))
        ...
        self.prod_tbl = make_table([tr("SKU"),tr("Name"),tr("Category"),tr("Cost"),tr("Selling Price"),tr("Stock")])
        ll.addWidget(self.prod_tbl)

        ll.addWidget(QLabel(tr("⚠️  Low Stock Alerts"), objectName="sec_title"))
        ...
        rl.addWidget(QLabel(tr("✏️  Edit Price & Cost"), objectName="sec_title"))
        note = QLabel(tr("Select a product row, then update its selling price and cost."))
        ...
        self.sel_sku.setPlaceholderText(tr("Auto-filled on row select"))
        self.sel_name = QLineEdit()
        self.sel_name.setPlaceholderText(tr("Product name (editable)"))
        ...
        f.addRow(tr("SKU:"),       self.sel_sku)
        f.addRow(tr("Name:"),      self.sel_name)
        f.addRow(tr("Unit Cost:"), self.new_cost)
        f.addRow(tr("Sale Price:"), self.new_price)
        rl.addLayout(f)

        btn_row = QHBoxLayout(); btn_row.setSpacing(8)
        self.upd_btn = QPushButton(tr("💾  Update Price")); self.upd_btn.setObjectName("primary")
        self.upd_btn.setCursor(Qt.PointingHandCursor); self.upd_btn.clicked.connect(self.handle_update_price)
        self.del_btn = QPushButton(tr("🗑  Delete Product")); self.del_btn.setObjectName("danger")
    ...
    def handle_update_price(self):
        ...
        if not sku: return QMessageBox.warning(self.page, tr("Select Product"), tr("Click a row first."))
        if self.new_price.value() <= 0: return QMessageBox.warning(self.page, tr("Invalid"), tr("Price must be > 0."))
        ...
        if ok:
            QMessageBox.information(self.page, "✅", tr("Product updated."))
            self.refresh()
            if self._refresh_cb: self._refresh_cb()
        else:
            QMessageBox.warning(self.page, tr("Error"), msg)
...
class ExpensesPage:
    def __init__(self):
        self.page, lay = page_container(tr("Expenses"))
        grid = QHBoxLayout(); grid.setSpacing(20)

        left = QWidget(); left.setObjectName("card")
        ll = QVBoxLayout(left); ll.setContentsMargins(16,16,16,16)
        ll.addWidget(QLabel(tr("📋  Expense History"), objectName="sec_title"))
        self.exp_tbl = make_table(["ID",tr("Category"),tr("Amount"),tr("Date"),tr("Description")])
        ll.addWidget(self.exp_tbl)

        row = QHBoxLayout()
        self.total_lbl = QLabel(tr("Total: 0 EGP"))
        ...
        right = QWidget(); right.setObjectName("card")
        rl = QVBoxLayout(right); rl.setContentsMargins(16,16,16,20); rl.setSpacing(14)
        rl.addWidget(QLabel(tr("➕  Add Expense"), objectName="sec_title"))
        f = QFormLayout(); f.setSpacing(10)
        ...
        f.addRow(tr("Category:"), self.exp_cat)
        f.addRow(tr("Amount:"), self.exp_amt)
        f.addRow(tr("Description:"), self.exp_desc)
        rl.addLayout(f)
        btn = QPushButton(tr("➕  Add Expense")); btn.setObjectName("primary")
    ...
    def handle_delete(self):
        ...
        if row < 0: return QMessageBox.warning(self.page, tr("Error"), tr("Select an expense."))
...
class ReportsPage:
    def __init__(self):
        self.page, lay = page_container(tr("Financial Reports"), tr("Segment Reporting & Analytics"))

        row = QHBoxLayout(); row.setSpacing(14)
        self.r_rev  = StatCard("💰", tr("Total Revenue"),  "—", "#3ecf8e")
        self.r_exp  = StatCard("💸", tr("Total Expenses"), "—", "#f06292")
    ...
    def refresh(self):
        ...
        types = [tr("Study"), tr("Gaming"), tr("Cinema")]
        self.type_tbl.setRowCount(len(types))
        for i, t in enumerate(types):
            amt = rev_type.get(t, 0)
            pct = (amt / total_r) * 100
            set_row(self.type_tbl, i, [f"{'📚' if t==tr("Study") else '🎮' if t==tr("Gaming") else '🎬'}  {t}",
                                        f"{amt:,.2f}", f"{pct:.1f}%"])
...
class SalesInvoicePage:
    def __init__(self):
        ...
        left = QWidget(); left.setObjectName("card")
        ll = QVBoxLayout(left); ll.setContentsMargins(16,16,16,16)
        ll.addWidget(QLabel(tr("📋  Sales Invoices"), objectName="sec_title"))
        ...
        self.detail_tbl = make_table([tr("Product"),tr("Qty"),tr("Price"),tr("Total")])
        ...
        row = QHBoxLayout()
        del_btn = QPushButton(tr("🗑 Delete")); del_btn.setObjectName("danger")
        del_btn.setCursor(Qt.PointingHandCursor); del_btn.clicked.connect(self.handle_delete)
        paid_btn = QPushButton(tr("✅ Mark Paid")); paid_btn.setObjectName("primary")
        paid_btn.setCursor(Qt.PointingHandCursor); paid_btn.clicked.connect(self.handle_paid)
        pdf_btn = QPushButton(tr("📄 Export PDF")); pdf_btn.setObjectName("secondary")
        ...
        rl.addWidget(QLabel(tr("🧾  Sales Invoice"), objectName="sec_title"))
    ...
    def refresh(self):
        ...
        for i, inv in enumerate(invs):
            ...
            if si: si.setForeground(QColor("#3ecf8e") if inv[4]==tr("Paid") else QColor("#ffa726"))
        self.detail_card.hide()
    ...
    def handle_save(self):
        ...
        if not cust: return QMessageBox.warning(self.page, tr("Error"), tr("Enter a customer name."))
        ...
        if requires_override:
            ...
            if not is_admin:
                ...
                pwd, ok = QInputDialog.getText(self.page, tr("Manager Override"), tr("Enter Admin Password to authorize selling below cost:"), QLineEdit.Password)
                ...
                conn.close()
                if not admin_user:
                    return QMessageBox.warning(self.page, tr("Error"), tr("Invalid admin password! Transaction blocked."))
                    
        if not items: return QMessageBox.warning(self.page, tr("Error"), tr("Add at least one product."))
    ...
    def handle_delete(self):
        row = self.inv_tbl.currentRow()
        if row < 0 or row >= len(self._inv_ids): return QMessageBox.warning(self.page, tr("Error"), tr("Select an invoice."))
        iid = self._inv_ids[row]
...
class PurchaseInvoicePage:
    def __init__(self):
        ...
        self.detail_title = QLabel(tr("Invoice Details")); self.detail_title.setObjectName("sec_title")
        dl.addWidget(self.detail_title)
        ...
        sup_lbl = QLabel(tr("Supplier")); sup_lbl.setStyleSheet("color:#8a9cc8; font-size:10px; font-weight:600; letter-spacing:1px;")
    ...
    def handle_save(self):
        ...
        if not sup: return QMessageBox.warning(self.page, tr("Error"), tr("Select a supplier."))
        errors = []; items = []
        for e in self.item_rows:
            name = e['name_fld'].text().strip()
            qty  = e['qty'].value()
    ...
    def handle_delete(self):
        row = self.inv_tbl.currentRow()
        if row < 0 or row >= len(self._inv_ids):
            return QMessageBox.warning(self.page, tr("Error"), tr("Select an invoice."))
        iid = self._inv_ids[row]
        reply = QMessageBox.question(self.page, tr("Confirm"), f"Delete invoice #{row+1}?",
                                     QMessageBox.Yes | QMessageBox.No)
        if reply == QMessageBox.Yes:
...
class AccountingPage:
    def __init__(self):
        ...
        jl.addWidget(QLabel(tr("📒  Journal Entries"), objectName="sec_title"))
        self.je_tbl = make_table(["#",tr("Date"),tr("Description"),tr("Entity"),tr("Reference"),tr("Total Debit")])
        ...
        self.je_ref    = QLineEdit(); self.je_ref.setPlaceholderText(tr("Reference (invoice#, etc.)..."))
        ...
        f.addRow(tr("Description:"),   self.je_desc)
        ...
        gl.addWidget(QLabel(tr("📖  General Ledger"), objectName="sec_title"))
        ...
        il.addWidget(QLabel(tr("💰  Income Statement"), objectName="sec_title"))
        sr = QHBoxLayout(); sr.setSpacing(14)
        self.is_rev = StatCard("💰",tr("Total Revenue"),tr("0 EGP"),"#3ecf8e")
        self.is_exp = StatCard("💸",tr("Total Expenses"),tr("0 EGP"),"#f06292")
        self.is_net = StatCard("📈",tr("Net Income"),tr("0 EGP"),"#5c9cf5")
        ...
        ev.addWidget(QLabel(tr("🔴  Expenses"), objectName="sec_title"))
        self.is_exp_tbl = make_table([tr("Account"),tr("Amount (EGP)")])
        ev.addWidget(self.is_exp_tbl)
        self.is_exp_total = QLabel(tr("Total: 0.00"))
        ...
        bl.addWidget(QLabel(tr("📊  Balance Sheet"), objectName="sec_title"))
        sr2 = QHBoxLayout(); sr2.setSpacing(14)
        self.bs_asset = StatCard("🏦",tr("Total Assets"),tr("0 EGP"),"#3ecf8e")
        self.bs_liab  = StatCard("📋",tr("Total Liabilities"),tr("0 EGP"),"#f06292")
        self.bs_eq    = StatCard("💎",tr("Total Equity"),tr("0 EGP"),"#5c9cf5")
        ...
        lv.addWidget(QLabel(tr("💎  Equity"), objectName="sec_title"))
        self.bs_e_tbl = make_table([tr("Account"),tr("Balance (EGP)")])
        lv.addWidget(self.bs_e_tbl)
        self.bs_e_total = QLabel(tr("Total: 0.00"))
    ...
    def refresh(self):
        ...
        if unpaid_purchase > 0:
            liab_rows.append((tr("Accounts Payable"), unpaid_purchase))
    ...
    def handle_add_je(self):
        ...
        if not desc: return QMessageBox.warning(self.page, tr("Error"), tr("Enter description."))
        ...
        if dr_amt != cr_amt:
            return QMessageBox.warning(self.page, tr("Error"), tr("Total Debit must equal Total Credit."))
    ...
    def handle_add_account(self):
        ...
        type_combo.addItems([tr("Asset"), tr("Liability"), tr("Equity"), tr("Revenue"), tr("Expense")])
        
        lay.addRow(tr("Account Name:"), name_edit)
        lay.addRow(tr("Account Type:"), type_combo)
        ...
        if dlg.exec_() == QDialog.Accepted:
            ...
            if not name:
                return QMessageBox.warning(self.page, tr("Error"), tr("Account name cannot be empty."))
...
class AccountStatementPage:
    def __init__(self):
        ...
        main_lay.setContentsMargins(30, 24, 30, 24); main_lay.setSpacing(16)
        h = QHBoxLayout()
        t = QLabel(tr("Account Statements")); t.setObjectName("title"); h.addWidget(t)
        h.addStretch()
        main_lay.addLayout(h)
        main_lay.addWidget(make_divider())
        ...
        fr = QHBoxLayout()
        fr.addWidget(QLabel(tr("👤 Customer:")))
        ...
        sr = QHBoxLayout(); sr.setSpacing(14)
        self.cs_total = StatCard("💰",tr("Total"),tr("0 EGP"),"#5c9cf5")
        self.cs_paid  = StatCard("✅",tr("Paid"),tr("0 EGP"),"#3ecf8e")
        self.cs_out   = StatCard("⏳",tr("Outstanding"),tr("0 EGP"),"#ffa726")
        ...
        sr2 = QHBoxLayout(); sr2.setSpacing(14)
        self.ss_total = StatCard("💰",tr("Total"),tr("0 EGP"),"#5c9cf5")
        self.ss_paid  = StatCard("✅",tr("Paid"),tr("0 EGP"),"#3ecf8e")
        self.ss_out   = StatCard("⏳",tr("Outstanding"),tr("0 EGP"),"#ffa726")
    ...
    def _load_customer(self):
        ...
        for i, r in enumerate(rows):
            ...
            if si: si.setForeground(QColor("#3ecf8e") if r[4]==tr("Paid") else QColor("#ffa726"))
...
class LoyaltyPage:
    ...
    def __init__(self):
        ...
        rl.addWidget(QLabel(tr("🔍  Customer Lookup"), objectName="sec_title"))
        ...
        rl.addLayout(f)
        self.redeem_info = QLabel(tr("100 pts = 1.00 EGP discount"))
    ...
    def _lookup(self):
        ...
        if not acc:
            ...
            self.spent_lbl.setText(tr("Spent: 0.00 EGP"))
            self.progress.setValue(0)
            self.next_lbl.setText(tr("1,000 pts to Silver"))
...
class SettingsPage:
    def __init__(self):
        self.page, lay = page_container(tr("⚙️  Settings"), tr("System Configuration"))
        grid = QHBoxLayout(); grid.setSpacing(20)

        left = QWidget(); left.setObjectName("card")
        ll = QVBoxLayout(left); ll.setContentsMargins(16, 16, 16, 16); ll.setSpacing(12)
        ll.addWidget(QLabel(tr("🔔  Notification Thresholds"), objectName="sec_title"))
        ...
        f.addRow(tr("Daily Revenue Target:"), self.rev_target)
        f.addRow(tr("Low Stock Threshold:"),  self.low_stock_t)
        f.addRow(tr("Session Alert (hrs):"),  self.sess_hrs)
        f.addRow(tr("Unpaid Invoice Alert:"), self.unpaid_cnt)
        ll.addLayout(f)

        ll.addWidget(make_divider())
        ll.addWidget(QLabel(tr("🎯  Loyalty Settings"), objectName="sec_title"))
        f2 = QFormLayout(); f2.setSpacing(10)
        ...
        save_btn = QPushButton(tr("💾  Save Settings")); save_btn.setObjectName("primary")
        ...
        right = QWidget(); right.setObjectName("card")
        rl = QVBoxLayout(right); rl.setContentsMargins(16, 16, 16, 16); rl.setSpacing(12)
        rl.addWidget(QLabel(tr("ℹ️  About AIS Hub"), objectName="sec_title"))
        for line in [
            ("📦", tr("Version"),    "3.0.0"),
            ("🐍", tr("Python"),     "3.12+"),
        ...
        db_lbl = QLabel(tr("Clears all transactional history (sessions, sales, expenses, invoices) to start a new year. Keeps master data like rooms, products, and customers."))
        db_lbl.setStyleSheet("font-size:11px; color:#f06292;"); db_lbl.setWordWrap(True)
        rl.addWidget(db_lbl)
        reset_db_btn = QPushButton(tr("📅  Start New Financial Year")); reset_db_btn.setObjectName("danger")
...
class BookingPage:
    def __init__(self):
        self.page, lay = page_container(tr("📅  Reservations & Bookings"))
        grid = QHBoxLayout(); grid.setSpacing(20)

        left = QWidget(); left.setObjectName("card")
        ll = QVBoxLayout(left); ll.setContentsMargins(16, 16, 16, 16)
        ll.addWidget(QLabel(tr("📋  All Bookings"), objectName="sec_title"))
        ...
        self.book_tbl = make_table(["#",tr("Room"),tr("Customer"),tr("Date"),tr("From"),tr("To"),tr("People"),tr("Deposit"),tr("Status")])
        ll.addWidget(self.book_tbl)

        btn_row = QHBoxLayout()
        cancel_btn = QPushButton(tr("❌  Cancel Booking")); cancel_btn.setObjectName("danger")
        cancel_btn.setCursor(Qt.PointingHandCursor); cancel_btn.clicked.connect(self.handle_cancel)
        edit_bk_btn = QPushButton(tr("✏️  Edit Booking")); edit_bk_btn.setObjectName("secondary")
        edit_bk_btn.setCursor(Qt.PointingHandCursor); edit_bk_btn.clicked.connect(self.handle_edit_booking)
        to_sess_btn = QPushButton(tr("▶  Convert to Session")); to_sess_btn.setObjectName("primary")
        ...
        right = QWidget(); right.setObjectName("card")
        rl = QVBoxLayout(right); rl.setContentsMargins(16, 16, 16, 16); rl.setSpacing(12)
        rl.addWidget(QLabel(tr("➕  New Booking"), objectName="sec_title"))
        ...
        f.addRow(tr("To:"),       self.bk_end)
        f.addRow(tr("People:"),   self.bk_ppl)
        f.addRow(tr("Deposit:"),  self.bk_dep)
        f.addRow(tr("Notes:"),    self.bk_notes)
        rl.addLayout(f)
        book_btn = QPushButton(tr("📅  Confirm Booking")); book_btn.setObjectName("primary")
    ...
    def refresh(self):
        ...
        for i, r in enumerate(rows):
            ...
            if si:
                c = {tr("Confirmed"): '#3ecf8e', tr("Cancelled"): '#f06292', tr("Completed"): '#5c9cf5'}.get(r[7], '#fff')
    ...
    def handle_book(self):
        ...
        if start >= end:
            return QMessageBox.warning(self.page, tr("Error"), tr("End time must be after start time."))
        ...
        if bk_date == today:
            ...
            if self.bk_start.time() < cur_time:
                return QMessageBox.warning(self.page, tr("Error"), tr("Cannot book a past time slot."))
